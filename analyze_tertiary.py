...
# Verificar se todos seguem um padrão similar
pattern = r'GCPJ\s+(\d+)'
matches = [re.search(pattern, str(v)) for v in gcpj_values]
valid_matches = [m.group(1) if m else None for m in matches]
print('\nGCPJs extraídos:')
for i, v in enumerate(valid_matches):
    print(f'  Original: {gcpj_values[i]} -> Extraído: {v}')

# Contar valores válidos
tertiary_df['gcpj_clean'] = tertiary_df['gcpj'].apply(
