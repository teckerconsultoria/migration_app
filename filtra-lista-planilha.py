...
class ExcelFilterApp:
    ...
    def select_sheet(self):
        ...
        if self.sheet_name in self.excel_sheets:
            try:
                ...
                self.coluna_var.set(colunas[0])  # Valor padrão
    ...
    def filtrar_planilha(self):
        ...
        df_filtrado = self.df[self.df[coluna].astype(str).isin(self.lista_valores)]

        save_path = filedialog.asksaveasfilename(defaultextension=".xlsx", filetypes=[("Excel", "*.xlsx")])
        if save_path:
            df_filtrado.to_excel(save_path, index=False)
